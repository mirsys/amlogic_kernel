...
static int set_disp_mode_auto(void)
{
	...
	if ((vic_ready != HDMI_Unkown) && (vic_ready == vic)
		...
		hdmi_print(IMP, SYS "[%s] ALREADY init VIC = %d\n",
			__func__, vic);
		hdmi_print(IMP, SYS "[%s] hdmitx_device.RXCap.IEEEOUI = %d\n",
			__func__, hdmitx_device.RXCap.IEEEOUI);
//
...
//		}
//
		/* fixme :force change to HDMI mode */
		hdmitx_device.HWOp.CntlConfig(&hdmitx_device,
			CONF_HDMI_DVI_MODE, HDMI_MODE);
		hdmi_print(IMP, SYS "force change to HDMI mode\n");
