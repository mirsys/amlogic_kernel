...
static int meson_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	case PWM_A:
		/*enable pwm_a*/
		aml_cbus_update_bits(PWM_MISC_REG_AB, 1<<0, 1);
		break;
	case PWM_F:
		/*enable pwm_f*/
		aml_cbus_update_bits(PWM_MISC_REG_EF, 2<<0, 1);
		break;
	}
...
static void meson_pwm_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	case PWM_A:
		/* disable pwm_a */
		aml_cbus_update_bits(PWM_MISC_REG_AB, 1 << 0, 0);
		break;
	case PWM_F:
		/* disable pwm_f */
		aml_cbus_update_bits(PWM_MISC_REG_EF, 2 << 0, 0);
		break;
	}
	return;
}
...
static int meson_pwm_probe(struct platform_device *pdev)
{
	...
	if (npwm == 2)
		strcpy(prop_name, "pwm1");
	else
		strcpy(prop_name, "pwm0");
